...
from trainers.hyptorch.nn import ToPoincare
from trainers.hyptorch.pmath import dist_matrix
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.ProSimOHyper.N_CTX
        ctx_init = cfg.TRAINER.ProSimOHyper.CTX_INIT
        ...
        if ctx_init:
            ...
            if cfg.TRAINER.ProSimOHyper.CSC:
        ...
        self.class_token_position = cfg.TRAINER.ProSimOHyper.CLASS_TOKEN_POSITION
        ...
        self.v_ctx = cfg.TRAINER.ProSimOHyper.V_CTX
...
class Hyperball(nn.Module):
    ...
    def norm(self, image_feature):
        b, n, c = image_feature.shape
        image_feature = image_feature.reshape(-1, c)

        text_feature = torch.zeros((1, c)).type_as(image_feature).to(image_feature.device)
        # text_feature = text_feature.unsqueeze(0)
        # print(image_feature.shape, text_feature.shape)
        logits = (
                -dist_matrix(image_feature, text_feature, c=self.e2p.c)
        )
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, images, image_features=None, local_image_features=None, max_list=None, min_list=None):
        ...
        if self.training:
            ...
            hyper_pos_local_image_features = self.hyperball(pos_local_image_features)
            hyper_neg_local_image_features = self.hyperball(pos_local_image_features)
            hyper_local_text_features = self.hyperball(local_text_features)
            hyper_neg_text_features = self.hyperball(neg_text_features)



            logit_scale = self.logit_scale.exp()

            hyper_logits_local = logit_scale * hyper_pos_local_image_features @ hyper_local_text_features.t()
            hyper_p2n_logits_local = logit_scale * hyper_neg_local_image_features @ hyper_local_text_features.t()
            hyper_n2p_logits_local = logit_scale * hyper_pos_local_image_features @ hyper_neg_text_features.t()
            hyper_neg_logits_local = logit_scale * hyper_neg_local_image_features @ hyper_neg_text_features.t()
            ...
            loss_div_2 = torch.nn.functional.cosine_similarity(hyper_neg_text_features[None, :, :],
                                                             hyper_neg_text_features[:, None, :], dim=-1)
            ...
            loss_div_2 = torch.sum(loss_div_2, dim=-1) / self.prompt_learner.num_neg_prompts
            loss_div_2 = torch.sum(loss_div_2, dim=-1) / (self.prompt_learner.num_neg_prompts - 1)
            loss_div = loss_div + loss_div_2

            return logits_local, hyper_logits_local, p2n_logits_local, hyper_p2n_logits_local, n2p_logits_local, hyper_n2p_logits_local,\
                   neg_logits_local, hyper_neg_logits_local, loss_div
            ...
            hyper_local_image_features = self.hyperball(local_image_features)
            hyper_local_text_features = self.hyperball(local_text_features)
            hyper_neg_text_features = self.hyperball(neg_text_features)
            ...
            hyper_logits_local = logit_scale * hyper_local_image_features @ hyper_local_text_features.t()
            hyper_neg_logits_local = logit_scale * hyper_local_image_features @ hyper_neg_text_features.t()


            return logits, logits_local, logits_local_vp, neg_logits_local, neg_logits_local_vp, hyper_logits_local, hyper_neg_logits_local


@TRAINER_REGISTRY.register()
class ProSimOHyper(TrainerX):
    """
    Extensible Local Prompts for Few-Shot Out-of-Distribution Detection (LOCAL-PROMPT).
    """

    def check_cfg(self, cfg):
        assert cfg.TRAINER.ProSimOHyper.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.ProSimOHyper.PREC == "fp32" or cfg.TRAINER.ProSimOHyper.PREC == "amp":
        ...
        train_parameters.append('hyperball')
        ...
        self.scaler = GradScaler() if cfg.TRAINER.ProSimOHyper.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.ProSimOHyper.PREC
        ...
        if prec == "amp":
            ...
            for i in range(num_pos):
                with autocast():
                    ...
                    output_local, hyper_output_local, p2n_output_local, hyper_p2n_output_local, n2p_output_local, hyper_n2p_output_local, \
                    neg_output_local, hyper_neg_output_local, loss_div = self.model(image, image_features,
                                                                                    local_image_features, max_list[i:i + 1,:])
                    ...
                    hyper_loss_local = self.calculate_loss_local(hyper_output_local, hyper_n2p_output_local, label)
                    loss_local_negative = self.calculate_loss_local_neg(neg_output_local, p2n_output_local, label)
                    hyper_loss_local_negative = self.calculate_loss_local_neg(hyper_neg_output_local, hyper_p2n_output_local, label)
                    loss = loss_local + hyper_loss_local + self.lambda_value * (loss_local_negative + hyper_loss_local_negative) + self.div_value * loss_div
        ...
        loss_summary = {
            ...
            "loss_local_neg": loss_local_negative.item() if loss_local_negative else 0,
            "hyper_loss_local": hyper_loss_local.item(),
            "hyper_loss_local_neg": hyper_loss_local_negative.item() if loss_local_negative else 0,
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output_global, output_local, _, _, _, _, _ = self.model_inference(input)
    ...
    def test_ood(self, data_loader, top_k, T):
        ...
        for batch_idx, (images, labels, *id_flag) in enumerate(tqdm(data_loader)):
            ...
            output, output_local, output_local_vp, neg_output_local, neg_output_local_vp, hyper_output_local, hyper_neg_output_local \
                = self.model_inference(images)

            output /= 100.0
            output_local /= 100.0
            neg_output_local /= 100.0
            output_local_vp /= 100.0
            neg_output_local_vp /= 100.0
            hyper_output_local /= 100.0
            hyper_neg_output_local /= 100.0
            ...
            N, C = output_local_vp.shape[1:]
            #print(output_local_vp.shape)
            smax_local_vp, indices = torch.topk((torch.exp(output_local_vp/ T) / \
                                     torch.sum(torch.exp(torch.cat((output_local_vp, neg_output_local_vp), dim=-1) / T),
                                               dim=-1, keepdim=True)).reshape(-1, N * C), k=top_k, dim=-1)
            all = N*C

            start_new = 196 * C
            end_new = 204 * C
            mask = (indices >= start_new) & (indices < end_new)
            cnt = mask.sum(dim=1)
            p = cnt.float().mean()/top_k
            print(p)
            # print(indices)
            mcm_local_score_vp = -to_np(torch.mean(smax_local_vp, dim=1))

            N, C = output_local.shape[1:]
            hyper_smax_local = torch.topk((torch.exp(hyper_output_local / T) / \
                                     torch.sum(torch.exp(torch.cat((hyper_output_local, hyper_neg_output_local), dim=-1) / T),
                                               dim=-1, keepdim=True)).reshape(-1, N * C), k=top_k, dim=-1)[0]
            hyper_mcm_local_score = -to_np(torch.mean(hyper_smax_local, dim=1))

            # print(uncertainty)
            local_prompt_score.append(mcm_global_score + 0.5 * mcm_local_score + 0.5 * hyper_mcm_local_score)


        return concat(mcm_score)[:len(data_loader.dataset)].copy(), concat(local_prompt_score)[
    ...
    def test_ood_imagenet(self, data_loader, top_k, T):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output, output_local, output_local_vp, neg_output_local, neg_output_local_vp, hyper_metric = self.model_inference(
                images)
            output /= 100.0
            output_local /= 100.0
            neg_output_local /= 100.0
            ...
            N, C = output_local.shape[1:]
            smax_local = torch.topk((torch.exp(output_local / T) / torch.sum(torch.exp(torch.cat((output_local, neg_output_local), dim=-1) / T),
                                                                             dim=-1, keepdim=True)).reshape(-1, N * C), k=top_k, dim=-1)[0]
            mcm_local_score = -to_np(torch.mean(smax_local, dim=1))

            N, C = output_local_vp.shape[1:]
            smax_local_vp = torch.topk((torch.exp(output_local_vp / T) /
                                        torch.sum(
                                            torch.exp(torch.cat((output_local_vp, neg_output_local_vp), dim=-1) / T),
                                            dim=-1, keepdim=True)).reshape(-1, N * C), k=top_k, dim=-1)[0]
            mcm_local_score_vp = -to_np(torch.mean(smax_local_vp, dim=1))

            hyper_metric = hyper_metric.squeeze(2)
            hyper_metric = F.softmax(hyper_metric, dim=-1)
            uncertainty = torch.topk(hyper_metric, k=top_k, dim=-1)[0]
            uncertainty = to_np(torch.mean(uncertainty, dim=1))
            # local_prompt_score.append(mcm_global_score + 1.0 * mcm_local_score + 0.0 * mcm_local_score_vp)
            local_prompt_score.append(uncertainty)
