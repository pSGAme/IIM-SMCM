...
class Hyperball(nn.Module):
    ...
    def distance(self, image_feature, text_feature):
        ...
        logits = (
                -dist_matrix(image_feature, text_feature, c=self.e2p.c) / 0.01
        )
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, images, image_features=None, local_image_features=None, max_list=None, min_list=None):
        ...
        if self.training:
            ...
            hyper_logits_local = self.hyperball.distance(hyper_pos_local_image_features, hyper_local_text_features)
            hyper_p2n_logits_local = self.hyperball.distance(hyper_neg_local_image_features, hyper_local_text_features)
            hyper_n2p_logits_local = self.hyperball.distance(hyper_pos_local_image_features, hyper_neg_text_features)
            hyper_neg_logits_local = self.hyperball.distance(hyper_neg_local_image_features, hyper_neg_text_features)
            ...
            hyper_metric = self.hyperball.norm(hyper_local_image_features)
            ...
            return logits, logits_local, logits_local_vp, neg_logits_local, neg_logits_local_vp, hyper_metric
...
class ProSimOHyper(TrainerX):
        ...
        if prec == "amp":
            ...
            for i in range(num_pos):
                with autocast():
                    ...
                    loss = loss_local + 5 * hyper_loss_local + self.lambda_value * (loss_local_negative + hyper_loss_local_negative) + self.div_value * loss_div
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output_global, output_local, _, _, _,_ = self.model_inference(input)
    ...
    def test_ood(self, data_loader, top_k, T):
        ...
        for batch_idx, (images, labels, *id_flag) in enumerate(tqdm(data_loader)):
            ...
            output, output_local, output_local_vp, neg_output_local, neg_output_local_vp, hyper_metric = self.model_inference(images)
            output /= 100.0
            output_local /= 100.0
            ...
            hyper_metric = F.softmax(hyper_metric, dim=-1)  # probability
            uncertainty = torch.topk(hyper_metric, k=top_k, dim=-1, largest=False)[0]
            # print(uncertainty)
