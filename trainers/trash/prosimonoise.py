        ...
        if self.training:
            ...
            for i in range(B):
               ...
               # neg[i] = images[min_list[0][i]][i]
                # print(pos.shape)
            with torch.no_grad():
                _, pos_local_image_featuresv2 = self.image_encoder(pos.type(self.dtype), visual_prompts)
            ...
            pos_local_image_featuresv2 = pos_local_image_featuresv2 / pos_local_image_featuresv2.norm(dim=-1, keepdim=True)
            ...
            logits_localv2 = logit_scale * pos_local_image_featuresv2 @ local_text_features.t()
            ...
            return logits_local, logits_localv2, p2n_logits_local, n2p_logits_local, neg_logits_local, loss_div
            ...
            return logits, logits_local, logits_local_vp, neg_logits_local, neg_logits_local_vp
...
class ProSimO(TrainerX):
    ...
    def calculate_loss_local(self, output_local, output_localv2, n2p_output_local, label):
        ...
        label_output_localv2 = output_localv2.gather(2, repeat(label, 'b -> b n 1', n=output_local.shape[1]))

        label_topk_output_local_pos = label_output_local.topk(k=self.top_k, dim=1)[1]
        label_topk_output_local = label_output_localv2.gather(1, label_topk_output_local_pos).squeeze()

        pos_topk_output_local = output_local.topk(k=self.top_k, dim=1)[1] # b, k, c
        pos_topk_output_local = output_localv2.gather(1, pos_topk_output_local).squeeze()
        #
        # pos_topk_ouput_local = output_localv2.gather(2, repeat(label, 'b -> b n 1', n=output_local.shape[1]))
        #
        neg_topk_output_local = n2p_output_local.topk(k=self.top_k, dim=1)[0]
        ...
        local_contrastive = torch.sum(
            torch.exp((label_topk_output_local - repeat(common_factor, 'b-> b k', k=self.top_k)) / self.T), dim=-1) / \
                            torch.sum(torch.exp((torch.cat((pos_topk_output_local, neg_topk_output_local),
                                                           dim=-1).reshape(-1,
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            for i in range(num_pos):
                with autocast():
                    output_local, output_localv2, p2n_output_local, n2p_output_local, neg_output_local, loss_div = self.model(image,
                                                                                                              image_features,
                    ...
                    loss_local = self.calculate_loss_local(output_local, output_localv2, n2p_output_local, label)
                    loss_local_negative = self.calculate_loss_local_neg(neg_output_local, p2n_output_local, label)
                    # p2n_output_local = output_local

                    # if loss_local_negative is None:
                    #     continue
                    # calculate total loss for ProSimO
    ...
    def test_ood(self, data_loader, top_k, T):
        ...
        for batch_idx, (images, labels, *id_flag) in enumerate(tqdm(data_loader)):
            ...
            output, output_local, output_local_vp, neg_output_local, neg_output_local_vp = self.model_inference(images)
            output /= 100.0
            output_local /= 100.0
            ...
            smax_local_vp = torch.topk((torch.exp(output_local_vp[:,:-8,:] / T) / \
                                     torch.sum(torch.exp(torch.cat((output_local_vp[:,:-8,:], neg_output_local_vp[:,:-8,:]), dim=-1) / T),
                                               dim=-1, keepdim=True)).reshape(-1, N * C), k=top_k, dim=-1)[0]
            mcm_local_score_vp = -to_np(torch.mean(smax_local_vp, dim=1))

            local_prompt_score.append(mcm_global_score + 0.0 * mcm_local_score + 1.0 * mcm_local_score_vp)
    ...
    def test_ood_imagenet(self, data_loader, top_k, T):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            smax_local_vp = torch.topk((torch.exp(output_local_vp[:,8:,:]/ T) /
                                       torch.sum(
                                            torch.exp(torch.cat((output_local_vp[:,8:,:], neg_output_local_vp[:,8:,:]), dim=-1) / T),
                                            dim=-1, keepdim=True)).reshape(-1, N * C), k=top_k, dim=-1)[0]
            mcm_local_score_vp = -to_np(torch.mean(smax_local_vp, dim=1))
